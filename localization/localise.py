...
def localize(colors,measurements,motions,sensor_right,p_move):
    ...
    for k in range(len(measurements)):
        ...
        p = sensors(p, measurements[k], sensor_right, colors)
    return p
...
def move(p, motions, p_move):
    ...
    for i in range(len(colors)):
        for j in range(len(colors[0])):
            q[i][j] = p_move * p[(i-motions[0]) % len(q)][(j-motions[1]) % len(q[0])]
            q[i][j] = q[i][j] + (1 - p_move) * p[i][j]
    return q
    
def sensors(p, measurements, sensor_right, colors):
    q = [[0.0 for row in range(len(colors[0]))] for col in range(len(colors))]
    s=0
    for i in range(len(colors)):
        for j in range(len(colors[0])):
